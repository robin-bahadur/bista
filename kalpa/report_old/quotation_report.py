...
class quotation_order(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(quotation_order, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            ...
            'get_sale_lines' : self._get_sale_lines,
    ...
    def _get_sale_lines(self, o):

        line_ids = []
        for x in o.order_line:
            line_ids.append(x.id)

        line_ids = self.pool.get('sale.order.line').search(self.cr, self.uid, [('id','in',line_ids)], order="ref_product")
        sale_line_objects = self.pool.get('sale.order.line').browse(self.cr, self.uid, line_ids)
        return sale_line_objects
...
report_sxw.report_sxw(
    ...
    'sale.order',
    'addons/kalpa/report/quotation_report.rml',
    parser=quotation_order
)
